...
export function createLabEditor(host: LabEditorHost): LabEditor {
  ...
  function renderEmitters(): string {
    ...
    const shapeModel = selected?.shape?.startsWith("model:") ? selected.shape.slice(6) : "";
    const grow = selected?.scaleOverLife ?? [1, 1];
    const alphaLife = selected?.alphaOverLife ?? [1, 1];
    const volume = selected?.volume ?? [0, 0, 0];
    const form = selected ? `<div class="lab-emitter-form">
        ...
        <div class="lab-panel-row"><span class="lab-field-label" title="What one particle is: a shape, or a whole voxel model from the catalog">Shape</span><select data-emitter-field="shape" title="cube: the default · flake: a flat plate (snow, dust, ice) · shard: a thin sliver · drop: a tall bead">${["cube", "flake", "shard", "drop"].map((shape) => `<option value="${shape}" ${(selected.shape ?? "cube") === shape ? "selected" : ""}>${shape}</option>`).join("")}</select><label title="Catalog model to use as the particle instead of the shape (a snowflake you authored here); empty = the shape">model<input type="text" data-emitter-field="shapeModel" value="${escapeHtml(shapeModel)}" class="lab-emitter-id" placeholder="— none —" /></label>${num("alpha", selected.alpha ?? 1, 0.05, "Transparency at birth: 1 solid, 0.15 barely there", 0)}</div>
        <div class="lab-panel-row"><span class="lab-field-label" title="Size and transparency from birth to death — a puff can come in small and sharp, swell, and thin away">Over life</span>${num("grow0", grow[0], 0.1, "Size at birth, as a multiple of size (0.5 = starts half)", 0)}${num("grow1", grow[1], 0.1, "Size at death (2 = swells to double, 0 = shrinks away). Equal to birth = no change", 0)}${num("alphaLife0", alphaLife[0], 0.1, "Transparency at birth, as a multiple of alpha", 0)}${num("alphaLife1", alphaLife[1], 0.1, "Transparency at death (0 = fades out completely)", 0)}</div>
        <div class="lab-panel-row"><span class="lab-field-label" title="Seed particles anywhere in a box this big instead of from one point (0,0,0 = a point) — cold air filling a cabinet">Volume</span>${[0, 1, 2].map((i) => `<input type="number" data-emitter-field="vol${i}" value="${volume[i]}" step="1" min="0" title="Emission box ${"xyz"[i]}, in voxels of this model" />`).join("")}</div>
  ...
  function onPanelChange(event: Event): void {
    ...
    if (input.dataset.emitterField !== undefined && selectedEmitter) {
      ...
      else if (field === "shape") patch.shape = value === "cube" ? undefined : value;
      else if (field === "shapeModel") { const model = value.trim().replace(/[^a-z0-9_-]/gi, ""); patch.shape = model ? `model:${model}` : undefined; }
      else if (field === "alpha") patch.alpha = Math.min(1, Math.max(0, n(1)));
      else if (field === "grow0" || field === "grow1") { const curve = [...(spec.scaleOverLife ?? [1, 1])] as [number, number]; curve[Number(field[4])] = Math.max(0, n(1)); patch.scaleOverLife = curve[0] === 1 && curve[1] === 1 ? undefined : curve; }
      else if (field === "alphaLife0" || field === "alphaLife1") { const curve = [...(spec.alphaOverLife ?? [1, 1])] as [number, number]; curve[Number(field[9])] = Math.max(0, n(1)); patch.alphaOverLife = curve[0] === 1 && curve[1] === 1 ? undefined : curve; }
      else if (field.startsWith("vol")) { const box = [...(spec.volume ?? [0, 0, 0])] as [number, number, number]; box[Number(field[3])] = Math.max(0, Math.round(n(0))); patch.volume = box.every((value) => value === 0) ? undefined : box; }
